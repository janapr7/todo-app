...
export default function CircleIcon({ isHovered, completed, dark }) {
  const outerStyles = {
    background: completed || isHovered ? 'linear-gradient(to right, hsl(192, 100%, 67%), hsl(280, 87%, 65%))' : 'hsl(236, 9%, 61%)',
  ...
  return (
    <div
      ...
      style={outerStyles}
    >
      {completed ? <img src={checkIcon} alt="Check Icon" /> : <div style={innerStyles}></div>}
    </div>
  );
}
